...
##-----------Updated version : 5 (modified 9th-December-2016) interactive & user-friendly.---##
...
#--------Checking the availability sysstat package..........#
if [ ! -x /usr/bin/mpstat ];
then
...
#-------function to fetch PCI device details--------#
pci_fun()
{
...
echo "$TDISK"|column -t|awk '{print $1"\t\t\t",$2"\t\t",$3$4}'|sed -e 's/[:|,]//g'|sort

#---------Printing each disk details depending on version-----------#
LDISK=$(echo "$TDISK"|awk '{print $2}'|sed 's/://g'|sort)
echo -e "\n\t\t Details Of Each Hard Drive(s) (local) Found" 
DISK=$(echo "$LDISK"|awk -F"/" '{print $3}')
for L in $(echo "$DISK")
do
{
    echo -e "\t$D$D"
...
case "$1" in 
	...
	;;
	*)
	echo "Usage: $0 {--memory|--cpu|--disk|--network|--details|--system|--health|--dump <PathForDumpFile> }"
